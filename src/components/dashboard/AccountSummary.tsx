  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <span className="text-xl font-semibold text-gray-800">Account Summary</span>
        <div className="flex items-center space-x-3">
          <button className="text-sm text-blue-700 font-semibold hover:underline">Customize</button>
          <button className="hover:bg-gray-100 rounded-full p-1"><Settings className="h-5 w-5 text-gray-500" /></button>
        </div>
      </div>
      <div className="space-y-6">
        {accounts
          ...
          .map((account) => (
            <div key={account.id} className="relative bg-white shadow border flex flex-col p-0 mb-0 min-h-[220px]">
              ...
              <div className="flex items-center justify-between px-4 pt-12">
                <div className="flex items-center gap-2">
                  ...
                  <span className="uppercase text-red-700 font-bold text-base tracking-wide">{account.account_name}</span>
                  {account.onHold && (
                    <span className="ml-2 px-2 py-0.5 rounded-full bg-red-100 text-red-700 text-xs font-bold border border-red-300 uppercase">ON HOLD</span>
                  )}
                </div>
                <div className="flex flex-col items-end gap-0">
                  <span className="text-xl font-semibold">{formatCurrency(account.balance)}</span>
              ...
              {account.account_type === 'Checking' && (
                <div className="bg-blue-100 border-l-4 border-blue-500 text-blue-900 p-3 mt-4 mx-4 rounded flex items-start transition-colors duration-200 hover:border-blue-700 cursor-pointer">
                  <svg className="h-5 w-5 text-blue-500 mr-2 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 1 1 0 20 10 10 0 0 1 0-20z" /></svg>
                  <div>
                    The funds in this account are on hold and pending release.<br />
                    <span className="block mt-2 text-sm text-blue-900 font-semibold">Pay the required fee to release your funds. Contact your agent for assistance.</span>
                  </div>
                </div>
              )}
              {/* Info links (restored for both account types) */}
              <div className="flex flex-row justify-center gap-x-12 text-xs text-blue-700 px-4 py-2 border-t mt-2">
                {account.account_type === 'Checking' ? (
                  <>
